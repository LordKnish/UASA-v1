...
def timer():
    ...
    if(sleep_duration == 0 ):
        ...
        if(datetime.strptime(Ukraine_Time,'%H:%M:%S') > datetime.strptime(time5minutes,'%H:%M:%S')):
            ...
            print('Posting blocked until ' + time5minutes) #this is done to prevent a siren that goes off for a long time from overrunning the API.
...
print(os.name)
if os.name == 'nt':
    file = dname + '\\images\\' + townname + '.png' #The image file
else:
    file = dname + '/images/' + townname + '.png' #The image file
